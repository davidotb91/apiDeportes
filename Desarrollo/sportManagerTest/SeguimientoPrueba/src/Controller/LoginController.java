...
public class LoginController {
    ...
    /* * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * */
    private Usuario validarEntrada(String nombre, String passWord, String ip)
            throws Exception {

        //Usuario usuario = getDao().validarEntrada(nombre, MD5.crypt(passWord));
        Usuario usuario = getDao().validarEntrada(nombre, passWord);
    ...
    public String validate() {
        ...
        try {
            ...
            Usuario usuario = this.validarEntrada(getName(), getPassword(),Context.getIp());

            if (usuario != null) {

                ((ControlSesion) Context.getBean("SesionBean")).setUsuario(usuario);
                usuario = this.validarEntrada("admin", "admin",Context.getIp());
                ((ControlSesion) Context.getBean("SesionBean")).isUsuario = true;
    ...
    public String profile() {
    	if ( ((ControlSesion) Context.getBean("SesionBean")).isUsuario)
    	{
	        current = (Usuario) ((ControlSesion) Context.getBean("SesionBean")).getUsuario();
    ...
    public String updateProfile() {
        try {
            Usuario medById = (Usuario) ((ControlSesion) Context.getBean("SesionBean")).getUsuario(); //getDao().find(current.getDocId(), false);
            if (current.getUsuaContrasena()!= null && !current.getUsuaContrasena().equals("")) {
                medById.setUsuaContrasena(MD5.crypt(current.getUsuaContrasena()));
            }
            //****************aqui modifico lo que voy a poder modificar
            //medById.setDocApellidos(current.getDocApellidos());
            //medById.setDocCedula(current.getDocCedula());
           //medById.set(current.getDocCelular());
            ...
            medById.setAudUsuModi(current.getUsuaId().toString());
    ...
    public void validateCurrentPassword(FacesContext arg0, UIComponent arg1, Object arg2)
        ...
        String currentPassword = ((Usuario) ((ControlSesion) Context.getBean("SesionBean"))
                .getUsuario()).getUsuaContrasena();
