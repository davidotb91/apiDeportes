...
public class DocenteHome {
	...
	public Docente findById(int id) {
		...
		try {
			Docente instance = (Docente) sessionFactory.getCurrentSession().get("entidades.Docente", id);
			if (instance == null) {
	...
	public List findByExample(Docente instance) {
		...
		try {
			List results = sessionFactory.getCurrentSession().createCriteria("entidades.Docente")
